...
class NetworkTopo( Topo ):
    def build( self, **_opts ):
        ...
        for h, s in [ (h1, s1), (h2, s2), (h3, s3), (h4, s4) ]:
            self.addLink( h, s )
def iperfTest(net, output_file, l_range=(3000, 300000), w_range=(128000, 256000)):
 
    # Start the iperf server on h2
    net['r1'].cmd('cd router1 && python3 switchd.py &')
    net['r2'].cmd('cd router2 && python3 switchd.py &')
    print("Checking connectivity from h1 to h2")
    pingResult = net['h1'].cmd(f"ping -c 50 {net['h2'].IP()}")
    print(pingResult)
    net['h2'].cmd('iperf3 -s &')
    
    with open(output_file, 'w') as f:
        f.write("BufferLength, TCPWindowSize, Throughput\n")

        for l_val in l_range:
            for w_val in w_range:
                info(f"Testing with -l {l_val} and -w {w_val}\n")
                result = net['h1'].cmd(f"iperf3 -c {net['h2'].IP()} -f m -b 1000M -l {l_val} -w {w_val}")
                info(f"Result: {result}\n") 
                matches = re.findall(r'\s([\d\.]+)\sMbits/sec', result)
                if matches:
                    throughput = matches[-1]
                    f.write(f"{l_val}, {w_val}, {throughput}\n")
                else:
                    f.write(f"{l_val}, {w_val}, N/A\n")

    net['h2'].cmd('kill %iperf3')
...
def run():
   ...
   # info( net[ 'r2' ].cmd( 'cd router2 && python3 switchd.py &' ) )
    ...
    iperfTest(net, "iperf_results.csv")  # Running iperfTest within the network
    CLI( net )
    net.stop()
